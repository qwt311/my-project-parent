...
public class TradePortalUtil {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(TradePortalUtil.class);

    private TradePortalUtil(){}

    /**
     * 获取系统流水号
     *  若欲指定返回值的长度or是否全由数字组成等,you can call {@link TradePortalUtil#getSysJournalNo(int, boolean)}
     * @return 长度为20的全数字
     */
     ...
     *  该方法默认以GBK解码
     *  若想自己指定字符集,可以使用<code>getStringFromIoBuffer(IoBuffer buffer, int size, String charset)</code>方法
     * @param size 所要读取的字节数
     */
    ...
    public static String getStringFromIoBuffer(IoBuffer buffer, int size, String charset){
        ...
        try {
            ...
            LOG.error("字符解码异常,自动切换第二种解码方式,本次的堆栈信息如下", e);
            try {
                result = new String(buffer.array(), charset);
            } catch (UnsupportedEncodingException ee) {
                LOG.error("字符解码异常,系统不支持该字符集[" + charset + "],本次的堆栈信息如下", ee);
            }
        }
        return result;
    }
     ...
     *  过滤规则:移除键名为"cert","hmac","signMsg"或者键值为null或者键值长度为零的键值对
     *  过滤结果:过滤完Map<String,String>后会产生一个字符串,其格式为[key11=value11|key22=value22|key33=value33]
     *  And the calls {@link TradePortalUtil#getHexSign(String,String,String,boolean)}进行签名
     ...
     *  It will Calls {@link TradePortalUtil#getBytes(String str, String charset)}
     *  若系统不支持<code>charset</code>字符集,则按照系统默认字符集进行转换
     *  若系统不支持<code>algorithm</code>算法,则直接返回""空字符串
     *  另外,commons-codec.jar提供的DigestUtils.md5Hex(String data)与本方法getHexSign(data, "UTF-8", "MD5", false)效果相同
    ...
    public static String encode(String chinese, String charset){
        ...
        try {
            ...
            LOG.error("编码字符串[" + chinese + "]时发生异常:系统不支持该字符集[" + charset + "]");
            return chinese;
        }
    }
    ...
    public static String decode(String chinese, String charset){
        ...
        try {
            ...
            LOG.error("解码字符串[" + chinese + "]时发生异常:系统不支持该字符集[" + charset + "]");
            return chinese;
        }
    }
     ...
     *  若想自己指定所补字符,可以使用<code>rightPadForByte(String str, int size, int padStrByASCII)</code>方法
     */
     ...
     *  若想自己指定所补字符,可以使用<code>leftPadForByte(String str, int size, int padStrByASCII)</code>方法
     */
